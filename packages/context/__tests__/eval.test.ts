...
describe("evaluate", () => {
  ...
  it.skipIf(!isDev)("should handle undefined properties", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      `[Lune] ERROR - Failed to execute expression "return(nonexistent)":`,
      expect.any(Error)
    );
  ...
  it.skipIf(!isDev)("should handle error cases gracefully", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      `[Lune] ERROR - Invalid expression: "return(syntax error)"`,
      expect.any(Error)
    );
    consoleSpy.mockRestore();
  });
...
describe("execute", () => {
  ...
  it.skipIf(!isDev)("should invalidate cache on execution error", () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      `[Lune] ERROR - Failed to execute expression "nonexistent.property.access":`,
      expect.any(Error)
    );
