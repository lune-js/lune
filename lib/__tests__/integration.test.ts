...
describe("integration tests", () => {
  ...
  describe("component-like behavior", () => {
    it("should handle scoped data in nested elements", async () => {
      container.innerHTML =
        '<div lu-scope="{ localCount: 0 }"><div>{{ localCount }}</div><button @click="localCount++">Increment</button></div>';
    ...
    it("should handle multiple independent instances", async () => {
      container.innerHTML =
        '<div id="app1"><div>{{ message }}</div><button @click="update">Update</button></div><div id="app2"><div>{{ message }}</div><button @click="update">Update</button></div>';
  ...
  describe("lifecycle and cleanup", () => {
    it("should clean up effects when unmounted", async () => {
      container.innerHTML = '<div lu-effect="trackEffect()"></div>';
    ...
    it("should handle conditional rendering", async () => {
      container.innerHTML = '<div lu-if="show">Visible Content</div><button @click="toggle">Toggle</button>';
  ...
  describe("error handling", () => {
    it.skipIf(!isDev)("should handle undefined expressions gracefully", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        `[Lune] ERROR - Failed to execute expression "return($s( undefinedVar ))":`,
        expect.any(Error)
      );
