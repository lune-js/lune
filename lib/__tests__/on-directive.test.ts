...
describe("on", () => {
  ...
  it("should handle lune:mounted lifecycle", async () => {
  ...
  it("should handle lune:unmounted lifecycle", () => {
    const el = document.createElement("div");
    const handler = vi.fn();
    ctx.scope.handler = handler;
  ...
  it("should return early when no arg provided", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('[Lune] ERROR - lu-on="obj" syntax is not supported.');
  ...
  it("should warn about deprecated mounted/unmounted hooks", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "[Lune] ERROR - Mounted and unmounted hooks now need to be prefixed with lune.",
      `Use '@lune:mounted="handler"' instead.`
    );
